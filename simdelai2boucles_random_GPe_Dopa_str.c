...
int main(){
	...
	dt=.1; tf1=2000; tf2=2000; tpsmoy=5;	gamma=0.3;																	//temps de simulation, gamma
	eps=0.; I_select_str=.001; I_ctx_mvt=0.05; /*.06; */ trampe=10; mvt_dur=100; t_str=20; t_mvt=500+mvt_dur/2;	//(mvt)
	pi=3.14159265; u=-2;	temp=0; mctx1=20; step=1;
	(*v1)=(2.0*ran1(&u)-1.0);
	(*v3)=(2.0*ran1(&u)-1.0);
	n=20; n1=n; nmoy=20; nunit=20; 
...
//	pcon[1]=1; pcon[2]=1; pcon[3]=1; pcon[4]=1; pcon[5]=1; pcon[6]=1; pcon[7]=1; pcon[8]=1; pcon[9]=1; pcon[10]=1; pcon[11]=1; pcon[12]=1; pcon[13]=1; 
	...
	Dgpigpe=0; Dgpestr=0; Dgpestn=0; Dstngpe=0;
	 ictx0=-0.11;  igpi0=-1.45; itha0=0.175;  istn0=0.08;  //-.13
//	igpi0=-.09; ictx0=-.08;  /*-.06; -0.12;*/ itha0=0.25; istn0=0.05; igpe0=0;   /*input currents*/
	sigma[1]=0.03; /* ctx */ sigma[2]=0.001; /* str */ sigma[3]=0.05; /* gpi */ sigma[4]=0.05; /* tha */ sigma[5]=0.02; /* stn */ sigma[6]=0.0; /* gpe */
	for(i=0;i<nstr;i++){
		th_str[i]=.018; //.022-.0045+0.005*gasdevbis(&u, v1, v3) ; //0.0175 +0.01*gasdevbis(&u, v1, v3) .035  0.01; //0188
	}
	for(i=0;i<nstr;i++){
		th_ctx[i]=0.0+.02*gasdevbis(&u, v1, v3); 
	}
	Ggpistr0=16-2; //13.5922; //12.14; //14; //
  ...
  for(D=0;D<11;D+=step){           //Calcul pour différent taux de dopa.
	...
	Gstrctx0=1.6/(1+exp(-2*(Dopa-1))); //1.6/(1+exp(-1.8*(Dopa-1))); //.8*Dopa; //.97/(1+exp(-2*(Dopa-.2))); //1.1/(1+exp(-2*(Dopa-.5))); //.75  0.83/(1+exp(-9*(Dopa-.6)));
	Gstrctxd0=Gstrctx0; 
	Gstrctxg0=Gstrctx0; 
	for(i=0;i<nctx;i++) {									/*définition des connections présentes*/
		for(j=0;j<nstr;j++) {
	...
	for(i=0;i<n;i++){
		istr[i]=-th_str[i]; //+.026*(1-2/(1+exp(-3*(Dopa-1)))); //1.2*.022*(1-2/(1+exp(-3*(Dopa-1)))); //0.06*(1-1.1/(1+0.1*exp(-3*(Dopa-1))));  //-th_str[i]+0.06*(1-1.1/(1+0.1*exp(-5*(Dopa-1)))); //0.005
